...
# Deploy the contracts (tokens and pool)
echo "Deploying contracts..."
npx hardhat run scripts/deployUniswapV3Pool.ts --network localhost

# Check if deployment files exist
if [ ! -f "deployments/hardhat/UniswapV3Pool.json" ]; then
    echo "Error: Pool deployment file not found!"
    cleanup
    exit 1
fi
...
# Run the test pool transactions
echo "Running test pool transactions..."
npx hardhat run scripts/test_pool_transactions.ts --network localhost
...
echo "Pool test environment is ready and running. Press Ctrl+C to stop..."
echo "Pool address can be found in deployments/hardhat/UniswapV3Pool.json"
wait